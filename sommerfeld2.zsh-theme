...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment magenta white "%{$fg[white]%(!.%{%F{white}%}.)%}$USER@%m%{$fg_no_bold[white]%}"
  else
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $has_diverged == false && $commits_ahead -gt 0 ]]; then
      if [[ $bgclr == 'red' || $bgclr == 'magenta' ]] then
        to_push=" $fg[white]↑$commits_ahead$fg[$fgclr]"
      else
        to_push=" $fg[black]↑$commits_ahead$fg[$fgclr]"
      fi
    fi
    if [[ $has_diverged == false && $commits_behind -gt 0 ]]; then to_pull=" $fg[magenta]↓$commits_behind$fg[$fgclr]"; fi
...
prompt_dir() {
  prompt_segment black black "%{$FG[255]%}%~%{$fg_no_bold[black]%}"
}
